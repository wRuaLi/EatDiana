...
(function(w) {
    ...
    function shareText(score) {
        ...
        if (mode === MODE_ENDLESS) {
            if (score <= 5) return '拜托，你很弱哎';
            if (score <= 8) return '为了圣嘉然，要继续努力捏';
            if (score <= 10)  return '圣嘉然看到了你的努力捏';
            if (score <= 15) return '这位更是个重量级';
            return '这位更是个几把';
        } else {
            if (score <= 49) return '拜托，你很弱哎';
            if (score <= 99) return '为了圣嘉然，要继续努力捏';
            if (score <= 149) return '圣嘉然看到了你的努力捏';
            if (score <= 199) return '这位更是个重量级';
            return '这位更是个几把';
        }
    }
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "吃掉小嘉然";
        if (cookie("keyboard")) {
